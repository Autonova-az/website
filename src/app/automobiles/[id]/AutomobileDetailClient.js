            ...
            <div className={styles.infoSection}>
                ...
                {(car.location || car.current_location) && (
                  <div className={styles.locationInfo}>
                    ...
                    {car.current_location && car.current_location !== car.location && (
                      <span className={styles.currentLocation}>📍 {t.automobileDetail?.location?.current || 'Hazırda'}: {car.current_location}</span>
                    )}
                  </div>
                )}
              </div>
              ...
              <div className={styles.priceSection}>
                <div className={styles.priceContact}>
                  <span className={styles.priceLabel}>{t.automobileDetail?.priceContact || 'Qiymət üçün əlaqə saxlayın'}</span>
                </div>
                {car.inStock && (
                  <div className={styles.stockInfo}>
                    <span className={styles.stockBadge}>{t.automobileDetail?.available || 'Mövcuddur'}</span>
                  </div>
                )}
              </div>
                ...
                <div className={styles.categorySpecs}>
                  {car.specifications.engine && (
                    <div className={styles.specItem}>
                      <span className={styles.specLabel}>{t.automobileDetail?.specifications?.engine || 'Mühərrik'}</span>
                      <span className={styles.specValue}>{car.specifications.engine}</span>
                    </div>
                  )}
                  {car.specifications.power && (
                    <div className={styles.specItem}>
                      <span className={styles.specLabel}>{t.automobileDetail?.specifications?.power || 'Güc'}</span>
                      <span className={styles.specValue}>{car.specifications.power}</span>
                    </div>
                  )}
                  {car.specifications.torque && (
                    <div className={styles.specItem}>
                      <span className={styles.specLabel}>{t.automobileDetail?.specifications?.torque || 'Fırlanma momenti'}</span>
                      <span className={styles.specValue}>{car.specifications.torque}</span>
                    </div>
                  )}
                  {car.specifications.acceleration && (
                    <div className={styles.specItem}>
                      <span className={styles.specLabel}>{t.automobileDetail?.specifications?.acceleration || 'Sürətlənmə'}</span>
                      <span className={styles.specValue}>{car.specifications.acceleration}</span>
                    </div>
                  )}
                  {car.specifications.topSpeed && (
                    <div className={styles.specItem}>
                      <span className={styles.specLabel}>{t.automobileDetail?.specifications?.maxSpeed || 'Maksimal sürət'}</span>
                ...
                <div className={styles.specCategory}>
                  ...
                  <div className={styles.categorySpecs}>
                    {car.specifications.range && (
                      <div className={styles.specItem}>
                        <span className={styles.specLabel}>{t.automobileDetail?.specifications?.range || 'Məsafə'}</span>
                        <span className={styles.specValue}>{car.specifications.range}</span>
                      </div>
                    )}
                    {car.specifications.battery && (
                      <div className={styles.specItem}>
                        <span className={styles.specLabel}>{t.automobileDetail?.specifications?.battery || 'Batareya'}</span>
                        <span className={styles.specValue}>{car.specifications.battery}</span>
                      </div>
                    )}
                    {car.specifications.charging && (
                      <div className={styles.specItem}>
                        <span className={styles.specLabel}>{t.automobileDetail?.specifications?.charging || 'Doldurma'}</span>
                        <span className={styles.specValue}>{car.specifications.charging}</span>
                      </div>
                    )}
                    {car.specifications.fuelConsumption && (
                      <div className={styles.specItem}>
                        <span className={styles.specLabel}>{t.automobileDetail?.specifications?.fuelConsumption || 'Yanacaq sərfiyyatı'}</span>
                        <span className={styles.specValue}>{car.specifications.fuelConsumption}</span>
                      </div>
                    )}
                  </div>
                </div>
              ...
              <div className={styles.specCategory}>
                ...
                <div className={styles.categorySpecs}>
                  {car.specifications.drivetrain && (
                    <div className={styles.specItem}>
                      <span className={styles.specLabel}>{t.automobileDetail?.specifications?.drivetrain || 'Ötürücü'}</span>
                      <span className={styles.specValue}>{car.specifications.drivetrain}</span>
                    </div>
                  )}
                  {car.specifications.seating && (
                    <div className={styles.specItem}>
                      <span className={styles.specLabel}>{t.automobileDetail?.specifications?.seating || 'Oturacaq sayı'}</span>
                      <span className={styles.specValue}>{car.specifications.seating}</span>
                    </div>
                  )}
                  {car.specifications.warranty && (
                    <div className={styles.specItem}>
                      <span className={styles.specLabel}>{t.automobileDetail?.specifications?.warranty || 'Zəmanət'}</span>
